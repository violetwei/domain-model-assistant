...
```bash
echo "console.log(JSON.parse(process.argv.slice(2).join('')).classDiagram.classes.map(c=>c._id+': '+c.name))" | node - `curl --silent --location --request GET 'http://localhost:8080/classdiagram/MULTIPLE_CLASSES/' --header 'Content-Type: application/json'`
```
